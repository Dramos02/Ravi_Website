...
function getPlanFile(e) {
  planFileItem = e.target.files[0];
  planFileName = planFileItem.name;
}

function getLocFile(e) {
  locFileItem = e.target.files[0];
  locFileName = locFileItem.name;
}

function uploadImage() {
  let storagePlanRef = firebase.storage().ref("images/" + planFileName);
  let uploadPlanTask = storagePlanRef.put(planFileItem);
  ...
  uploadPlanTask.on(
    ...
    () => {
      uploadPlanTask.snapshot.ref.getDownloadURL().then((url) => {
        console.log("URL", url);
      })
    }

  let storageLocRef = firebase.storage().ref("images/" + locFileName);
  let uploadLocTask = storageLocRef.put(locFileItem);
  ...
  uploadLocTask.on(
    ...
    () => {
      uploadLocTask.snapshot.ref.getDownloadURL().then((url) => {
        console.log("URL", url);
      })
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setPlanFileName() {
    var planFile = document.getElementById('pic_plan');
    var planFileName = planFile.name
    var planNameHolder = document.getElementById('pic_plan_name');
    planNameHolder.value = "images/" + planFileName;
  }

  function setLocFileName() {
    var locFile = document.getElementById('pic_loc');
    var locFileName = locFile.name
    var locNameHolder = document.getElementById('pic_loc_name');
    locNameHolder.value = "images/" + locFileName;
  }
